...
describe('Result', () => {
  describe('unwrapOr', () => {
    ...
    it('should unwrap the error of Err variant properly', () => {
      ...
      expect(
        result.unwrapOrElse(
          (num) => `Some Error happened when processing: ${num.message}`,
        ),
  ...
  describe('map', () => {
    it('should map Ok variant properly', () => {
      const sum = Result.Ok([1, 3, 4])
        .map((arr) => arr.reduce((total, current) => total + current, 0))
        .map((num) => num + 2);
  ...
  describe('tryCatch', () => {
    ...
    it('should return the correct value when working with the result', () => {
      ...
      const parsePkg = (pkgStr: string) => tryCatch(() => JSON.parse(pkgStr));
    ...
    it('should return Err if Promise rejects', async () => {
      ...
      const batman = (await async(() => getMovies('1')))
        .andThen((movies) => {
          ...
          return batman ? Result.Ok(batman) : Result.Err(Error('Not Found'));
        })
        .andThen((batman) => Result.Ok(`${batman} is weaker than superman`));
